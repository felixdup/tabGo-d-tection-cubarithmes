public class DetectionCube {
  ...
  DetectionCube(PImage src) {
    ...
    recherche();
    println("nombre de cube trouvés : "+ mesCubes.size());
  }
  ...
  /**
   * recherche la liste des points et des cubes; 
   */
  void recherche() {
  ...
  /**
     ...
     */
  ArrayList<Point> cherchePoint(PImage monImage, color couleurCible) {
    ...
    for (int x = 0; x < monImage.width; x++) {
      for (int y = 0; y < monImage.height; y++ ) {
        ...
        if (isNearCouleur(couleurCible, loc, monImage)) {
          ...
          PixelScore monPixel = new PixelScore(x, y, calculScore(x, y, couleurCible, monImage));

          if (listePoints.size() == 0) {           // si pas encore de blobs on en crée un et on ajoute le point
            listePoints.add(new Point(monPixel));
          } else {                                // on parcourt les blobs si le point est proche on l'agglomere au blob

            for (Point monPoint : listePoints) {   
              if (monPoint.isNear(monPixel)) {
  ...
  /**
     ...
     */
  ArrayList<Cube> chercheCube(ArrayList<Point> listePoints, int tailleMin, int tailleMax) {
    ArrayList<Cube> listeCubes = new ArrayList<Cube>();

    ArrayList<Point> trouve = new ArrayList<Point>();
  ...
  float calculScore(int x, int y, color couleurCible, PImage monImage) {
    ...
    int taille = 8;
    int s =0;
    for (int xrelatif = x-taille; xrelatif<x+taille; xrelatif++) {
      for (int yrelatif = y-taille; yrelatif<y+taille; yrelatif++) {
