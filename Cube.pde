...
class Cube {
  ...
  Cube( PVector rp1, PVector rp2, PVector rp3, PVector rp4) {
    ...
    if (valide) {
      ...
      calculScore();
      valide = couleurValide();
    }   
  }

  boolean estValide() {
    return angleHorizontalValide() && diagonaleValide() && angleVerticalValide() && opposeValide() && ordreValide() && opposeTailleValide() && rapportValide();
  }
  ...
  /**
   * Détermine si les côtés opposés sont de environs de même longueur
   * permet d'éviter les trapèzes
   * @return true si pour chaque côtés opposés du cube la différence en taille est <20, sinon false
   */
  boolean opposeTailleValide() {
    // println(abs(dist(p1.x, p1.y, p2.x, p2.y)-dist(p3.x, p3.y, p4.x, p4.y)));
    return abs(dist(p1.x, p1.y, p2.x, p2.y)-dist(p3.x, p3.y, p4.x, p4.y)) < 20 && abs(dist(p1.x, p1.y, p4.x, p4.y)-dist(p2.x, p2.y, p3.x, p3.y)) < 20 ;
  }
   ...
   */
  boolean diagonaleValide() {
   // println("diff"+abs(dist(p1.x, p1.y, p3.x, p3.y)-dist(p2.x, p2.y, p4.x, p4.y))+"  : "+dist(p1.x, p1.y, p3.x, p3.y)*.2+dist(p1.x, p1.y, p3.x, p3.y)*.2);
     
    return abs(dist(p1.x, p1.y, p3.x, p3.y)-dist(p2.x, p2.y, p4.x, p4.y)) < dist(p1.x, p1.y, p3.x, p3.y)*.2+dist(p1.x, p1.y, p3.x, p3.y)*.2;
  }
   /**
   * Détermine si le quadrilatère est croisé
   * permet d'éviter les quadrilatères croisés
   * @return true si non croisé, sinon false
   */
  boolean opposeValide() {
    return ((p2.x - p1.x) * (p4.x - p3.x) <0 && (p3.y - p2.y) * (p1.y - p4.y) < 0);
  }
  /**
   * Détermine si est trop incliné
   * permet d'éviter les trapèzes
   * @return true si différence <50, sinon false
   */
  boolean angleHorizontalValide() {
    return abs(abs((p2.y - p1.y)) - abs((p4.y - p3.y)))< 50;
  }
  /**
   * Détermine si est trop incliné
   * permet d'éviter les trapèzes
   * @return true si différence <50, sinon false
   */
  boolean angleVerticalValide() {
    return abs(abs((p3.y - p2.y)) - abs((p1.y - p4.y)))< 50;
  }
  /**
   * Détermine si les points du cubes sont bien ordonnés et de la manière voulu
 ...
 /**
  ...
  void calculCoordoneesPoints() {
    ...
    largeur = max.x - min.x;
    hauteur = max.y - min.y;
    c = new PVector(p1.x+largeur/2, p1.y+hauteur/2);
  }
 ...
 /**
  ...
  void dessineCercle() {
    ...
    circle(src.width+300+m1.x, m1.y, 30);
    circle(src.width+300+m2.x, m2.y, 30);
    circle(src.width+300+m3.x, m3.y, 30);
    circle(src.width+300+m4.x, m4.y, 30);
  }
 ...
 /**
  ...
  void dessineCube() {
    ...
    textSize(50);
    fill(255, 255,255);
    text(monCharacter, decallage+c.x, c.y);
  }
 /**
